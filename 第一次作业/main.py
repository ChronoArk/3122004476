...
def jaccard(model,reference):#refenrence为原文件，model为抄袭文件
    # 将中文字符串用jieba进行精确分词
    terms_refenrence = jieba.cut(reference,cut_all=False) #分词后赋值给变量
...
def file():
    if len(sys.argv) != 4:
        print("发生错误。正确格式：python main.py 原文文件路径 抄袭文件路径 输出文件路径")
        sys.exit(1)
    ...
    similarity = jaccard(f1,g1) #计算相似度

    with open(sys.argv[3], 'a', encoding='utf-8') as f:
        f.write( "{:.2f}\n\n".format(similarity) ) #将相似度写入答案文件中
